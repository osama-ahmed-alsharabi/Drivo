...
class EditServiceProviderProfileScreen extends StatefulWidget {
  ...
  const EditServiceProviderProfileScreen({super.key, required this.provider});
...
class _EditServiceProviderProfileScreenState
  ...
  void _loadInitialData() {
    ...
    _nameController.text = provider['facility_name'] ?? '';
    _descriptionController.text = provider['description'] ?? '';
    _addressController.text = provider['address'] ?? '';
    _phoneController.text = provider['phone_number'] ?? '';
    _emailController.text = provider['email'] ?? '';

    if (provider['latitude'] != null && provider['longitude'] != null) {
      _selectedLocation = LatLng(
        provider['latitude'] as double,
        provider['longitude'] as double,
      );
    }

    // Parse business hours from stored strings
    _parseBusinessHours(provider);
  }

  void _parseBusinessHours(Map<String, dynamic> provider) {
    try {
      _businessHours['sundayToThursday'] =
          _parseTimeString(provider['opening_hours_sun_thu']);
      _closingHours['sundayToThursday'] =
          _parseTimeString(provider['closing_hours_sun_thu']);
      _businessHours['friday'] =
          _parseTimeString(provider['opening_hours_fri']);
      _closingHours['friday'] = _parseTimeString(provider['closing_hours_fri']);
      _businessHours['saturday'] =
          _parseTimeString(provider['opening_hours_sat']);
      _closingHours['saturday'] =
          _parseTimeString(provider['closing_hours_sat']);
    } catch (e) {
  ...
  TimeOfDay _parseTimeString(String? timeString) {
    if (timeString == null) return const TimeOfDay(hour: 10, minute: 0);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<EditServiceProviderProfileCubit,
        EditServiceProviderProfileState>(
      listener: (context, state) {
        if (state is EditServiceProviderProfileError) {
          CustomSnackbar(
            context: context,
            snackBarType: SnackBarType.fail,
            label: state.message,
          );
        } else if (state is EditServiceProviderProfileSuccess) {
          CustomSnackbar(
            context: context,
            snackBarType: SnackBarType.success,
            label: "تم تحديث البيانات بنجاح",
          );
          Navigator.pop(context, true);
        }
      },
      child: BlocBuilder<EditServiceProviderProfileCubit,
          EditServiceProviderProfileState>(
          ...
          return Directionality(
            ...
            child: Scaffold(
              appBar: AppBar(
                ...
                title: const Text("تعديل بيانات المطعم"),
                actions: [
                  ElevatedButton(
              ...
              body: ModalProgressHUD(
                inAsyncCall: state is EditServiceProviderProfileLoading,
                opacity: 0.5,
                            ...
                            Container(
                              ...
                              child: _coverImageFile != null
                                  ? Image.file(_coverImageFile!,
                                      fit: BoxFit.cover)
                                  : widget.provider['cover_image_url'] != null
                                      ? CachedNetworkImage(
                                          imageUrl: widget
                                ...
                                child: ClipOval(
                                  child: _profileImageFile != null
                                      ? Image.file(_profileImageFile!,
                                          fit: BoxFit.cover)
                                      : widget.provider['logo_url'] != null
                                          ? CachedNetworkImage(
                                              imageUrl:
                                                  widget.provider['logo_url'],
                                              fit: BoxFit.cover,
  ...
  void _saveChanges() {
    if (_formKey.currentState!.validate()) {
      ...
      context.read<EditServiceProviderProfileCubit>().updateProfile(
            ...
            currentLogoUrl: widget.provider['logo_url'],
            currentCoverUrl: widget.provider['cover_image_url'],
          );
    }
  }
}
